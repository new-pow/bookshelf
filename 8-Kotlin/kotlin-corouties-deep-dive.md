...
- 결국 코루틴 프로그래밍을 할 때 **채널은 직접 사용하지 않더라도, 대부분의 상위 API에서 간접적으로 사용되는 기본 통신 메커니즘**.
#### 생산자-소비자 시나리오와 채널의 필요성

- 여러 개의 생산자(Producer) 코루틴이 데이터를 만들고, 여러 소비자(Consumer) 코루틴이 이를 처리하는 구조는 흔한 동시성 패턴입니다.  
- 이때 단순히 동시 큐(Concurrent Queue)를 사용하면 문제가 생김.
	- 큐가 비어 있으면 소비자는 쓸데없이 CPU 시간을 소모하며 대기하고, 반대로 생산자가 너무 빠르면 큐가 과도하게 커져 메모리를 잠식할 수 있습니다.

- 이런 문제를 해결하기 위해 등장한 것이 **코루틴 채널**입니다.  
	- 채널은 `send`와 `receive` 함수를 통해 데이터를 주고받으며, **필요할 때 자동으로 일시 중단(Suspension)** 함.  
	- 채널이 비어 있을 때 `receive()`를 호출하면 소비자는 새 데이터가 들어올 때까지 자동으로 멈추고,  
	- 소비할 수 있는 소비자가 없을 때 `send()`를 호출한 생산자 역시 잠시 중단됩니다.  
	- 이 메커니즘 덕분에 채널은 큐처럼 무한히 커지지 않고, **생산자-소비자 간 균형을 자연스럽게 유지**할 수 있음.
### 랑데부 채널(Rendezvous Channel)의 구조와 동작
- **랑데부 채널(Rendezvous Channel)**이라 부릅니다.  
- 이 채널에서는 생산자와 소비자가 만날때까지 기다림.  생산자가 데이터를 보내면 두 코루틴이 “랑데부”를 이루어 데이터를 교환하고 동시에 다시 실행됩니다.
- 이런 구조 덕분에 **메모리 낭비 없이 효율적인 통신이 가능합니다.
- 이 채널의 장점
	- 단일 스레드에서도 빠르게 동작
	- 메모리 사용량이 적음
	- 높은 부하에서도 확장 가능
...
- `send`와 `receive`는 내부적으로 슬롯을 예약하고, 상태 전이를 통해 데이터를 교환합니다. 이 방식으로 Kotlin 채널은 이전 구현보다 **최대 4배 빠르고, 메모리 사용량도 훨씬 줄었습니다.**

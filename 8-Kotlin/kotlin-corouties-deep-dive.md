...
---
# 16장. 채널
- 코루틴끼리의 통신을 위한 기본적인 방법 채널 API
- 송신자와 수신자 수에 제한이 없음. 채널을 통해 전송된 모든 값은 단 한번만 받을 수 있음.
	- 채널 양 끝에 각 하나의 코루틴만 있는 경우가 일반적임.
- Channel 은 다음의 인터페이스를 구현함.
	- SendChannel : 원소를 보내거나 (혹은 더하거나) 채널을 닫음.
...
- 원소를 공평하게 배분함.
- consumeEach 는 여러개의 코루틴이사용하기에 안전하지 않ㅇ므로 for 루프를 사용한다.
- 원소는 공평하게 배분된다.
- 채널은 원소를 기다리는 코루틴들을 FIFO 큐로 가지고 있으므로 순차적으로 원소를 받는다.
## 팬인 (Fan-in)
여러 개의 코루틴이 하나의 채널로 원소를 전송할 숭 ㅣㅆ다.
다수의 채널을 하나의 채널로 합쳐야할 경우, fanIn 을 사용한다. (여러개의 받아야하는 채널을 하나로 합쳐서 하나의 채널로 도착할 수있도록 함.)
## 파이프라인
- 한 채널로부터 받은 원소를 다른 채널로 전송하는 경우, 파이프라인이라고 함.
## 통신의 기본형태로서의 채널
- 서로 다른 코루틴이 통신할 때 유용하다.
## 실제 사용 예
- 데이터가 한쪽에서 생성되고, 다른쪽에서 데이터를 처리할 때 사용한다.
- 하지만 대부분의 경우, 채널과 플로우가 합쳐진 `channelFlow`, `callbackFlow`를 사용하는 것이 더 좋다.
- 순수한 형태의 채널은 좀더 복잡한 처리를 요하는 경우에 유용하다.
	- 여러 설정을 해야하는 경우?
---
- 채널이 중요한 이유
	- 대부분의 경우 우리는 `Flow` 같은 상위 수준 API를 사용하기 때문에, 채널을 직접 다룰 필요가 없음.
...
## 생산자-소비자 시나리오와 채널의 필요성

여러 개의 생산자(Producer) 코루틴이 데이터를 만들고, 여러 소비자(Consumer) 코루틴이 이를 처리하는 구조는 흔한 동시성 패턴입니다.  
이때 단순히 동시 큐(Concurrent Queue)를 사용하면 문제가 생깁니다.  
큐가 비어 있으면 소비자는 쓸데없이 CPU 시간을 소모하며 대기하고, 반대로 생산자가 너무 빠르면 큐가 과도하게 커져 메모리를 잠식할 수 있습니다.

이런 문제를 해결하기 위해 등장한 것이 **코루틴 채널**입니다.  
채널은 `send`와 `receive` 함수를 통해 데이터를 주고받으며, **필요할 때 자동으로 일시 중단(Suspension)** 합니다.  
즉, 채널이 비어 있을 때 `receive()`를 호출하면 소비자는 새 데이터가 들어올 때까지 자동으로 멈추고,  
소비할 수 있는 소비자가 없을 때 `send()`를 호출한 생산자 역시 잠시 중단됩니다.  
이 메커니즘 덕분에 채널은 큐처럼 무한히 커지지 않고, **생산자-소비자 간 균형을 자연스럽게 유지**합니다.

---

## 랑데부 채널(Rendezvous Channel)의 구조와 동작

Kotlin 코루틴의 기본 채널은 **랑데부 채널(Rendezvous Channel)**이라 부릅니다.  
이 채널에서는 생산자와 소비자가 “만날 때까지” 서로 기다립니다.  
예를 들어 소비자가 먼저 `receive()`를 호출하면, 생산자가 데이터를 `send()`할 때까지 일시 중단됩니다.  
생산자가 데이터를 보내면 두 코루틴이 “랑데부”를 이루어 데이터를 교환하고 동시에 다시 실행됩니다.  
이런 구조 덕분에 **메모리 낭비 없이 효율적인 통신이 가능**합니다.

이 채널을 구현하기 위해서는 몇 가지 조건이 필요합니다.
...
기존 방식처럼 전역 잠금을 사용하는 큐는 단일 스레드에서는 빠르지만 확장성이 떨어집니다.  
`ConcurrentLinkedQueue`는 확장 가능하지만 메모리 오버헤드와 캐시 미스가 많습니다.
...
이 덕분에 단일 스레드에서도 빠르고, GC(가비지 컬렉션) 친화적이며, 메모리 효율이 뛰어납니다.

`send`와 `receive`는 내부적으로 슬롯을 예약하고, 상태 전이를 통해 데이터를 교환합니다.  
이 방식으로 Kotlin 채널은 이전 구현보다 **최대 4배 빠르고, 메모리 사용량도 훨씬 줄었습니다.**

---

## 버퍼링된 채널(Buffer Channel)의 역할과 구조

랑데부 채널은 항상 일시 중단을 동반하지만,  
버퍼링된 채널은 **정해진 용량만큼은 일시 중단 없이 데이터를 전송할 수 있습니다.**

예를 들어 용량이 1인 버퍼 채널에서는 첫 번째 생산자가 데이터를 넣으면 바로 완료되지만,  
두 번째 생산자는 버퍼가 가득 찰 때까지 대기합니다.  
소비자가 데이터를 꺼내면 대기 중이던 생산자가 즉시 재개되어 다음 데이터를 넣습니다.

버퍼링된 채널 역시 랑데부 채널과 동일한 구조를 기반으로 하되,  
**가상의 버퍼 및 추가적인 카운터를 통해 상태를 관리**합니다.  
여기에 코루틴의 취소(cancellation) 상황까지 처리해야 하므로 구현이 더 복잡하지만,  
최신 버전의 채널은 고부하 상황에서도 안정적으로 확장되도록 설계되었습니다.

---

## 정리

정리하자면, Kotlin 코루틴의 채널은 단순한 큐가 아닌 **코루틴 간의 안전하고 효율적인 통신 메커니즘**입니다.  
직접 사용할 일이 많지 않더라도, `Flow`, `channelFlow`, `merge`, `RxJava 통합` 등 다양한 곳에서 이미 내부적으로 활용되고 있습니다.  
채널의 원리를 이해하는 것은 곧 **코루틴의 내부 동작과 성능 최적화를 이해하는 첫걸음**입니다.

현재 Kotlin은 **무제한 채널, 병합 채널 등 다양한 채널 변형**을 제공하며,  
모두 동일한 설계 철학을 기반으로 동작합니다.  
보다 깊이 있는 내용을 알고 싶다면, 앞으로 열릴 **코펜하겐 PDI 컨퍼런스에서 진행될 스케줄러·채널·뮤텍스·세마포어 강연**을 참고해 보시길 추천드립니다.

---

## Q&A 하이라이트

- **여러 생산자가 대기 중인 경우**: 채널은 **선입선출(FIFO)** 방식으로 첫 번째 생산자를 우선 처리합니다.
    
- **성능 테스트**: 64코어 AWS 인스턴스에서 실험한 결과, 코어 수를 초과하는 스레드를 늘려도 효율이 개선되지 않았습니다.
    
- **세그먼트 크기 선택**: 일반적으로 16~64 범위가 적절하며, **32 크기 세그먼트가 가장 균형적**으로 작동했습니다.
    
- **`ChannelFlow` 버퍼 크기 이슈**: 기본 버퍼 크기(64)는 현재 변경이 불가능하지만, GitHub 이슈를 통해 개선 요청을 받을 예정입니다.
    
- **Java의 BlockingQueue와의 비교**: Java의 차단 큐는 동기화를 위해 전역 잠금을 사용하므로, Kotlin의 버퍼 채널만큼 확장되지 않습니다.
    

---

이 글은 JetBrains의 니키타가 진행한 “Kotlin Coroutine Channels Explained” 강연을 기반으로 요약 및 재구성한 내용입니다.
---
# 17장. 셀렉트

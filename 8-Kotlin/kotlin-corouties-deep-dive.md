...
| 단계  | 동작                         | 내부 상태                          |
| --- | -------------------------- | ------------------------------ |
| ①   | 생산자가 `send()` 호출           | `sendIndex` = 101, 슬롯 예약       |
| ②   | 슬롯 상태를 `SUSPEND_SEND` 로 변경 | 데이터 저장, 소비자 기다림                |
| ③   | 소비자가 `receive()` 호출        | `receiveIndex` = 101, 동일 슬롯 예약 |
| ④   | CAS로 상태를 `FULFILLED` 로 변경  | 데이터 전달, 두 코루틴 재개               |
| ⑤   | 슬롯 재사용 또는 폐기               | 세그먼트 메모리 관리 수행                 |

#### 버퍼링된 채널(Buffer Channel)의 역할과 구조
- 랑데부 채널은 항상 일시 중단을 동반하지만,  버퍼링된 채널은 **정해진 용량만큼은 일시 중단 없이 데이터를 전송할 수 있음.**
- 버퍼링된 채널 역시 랑데부 채널과 동일한 구조를 기반으로 하되,  **가상의 버퍼 및 추가적인 카운터를 통해 상태를 관리**합니다.
#### 마무리
정리하자면, Kotlin 코루틴의 채널은 단순한 큐가 아닌 **코루틴 간의 안전하고 효율적인 통신 메커니즘**임.
직접 사용할 일이 많지 않더라도, `Flow`, `channelFlow`, `merge`, `RxJava 통합` 등 다양한 곳에서 이미 내부적으로 활용되고 있음.  
현재 Kotlin은 **무제한 채널, 병합 채널 등 다양한 채널 변형**을 제공하고 있는데, 모두 동일한 설계 철학을 기반으로 동작합니다.  
